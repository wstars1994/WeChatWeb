...
class Login():
    ...
    def get_qrcode(self,path="D:/1.jpg"):
        ...
        result = HttpClient().rq(uuid_url);
        uuid_msg=ResultCheck().uuid_check(result);
        if uuid_msg[0]==0:
    ...
    def login(self,qr_uuid):
        while(1):
            ...
            codeList = ResultCheck().first_login_check(result);
            code=int(codeList[0]);
            if code==201 :
    ...
    def __final_login(self,final_login_url):
        ...
        result = ResultCheck().final_login_check(result);
        if result[0]==0:
